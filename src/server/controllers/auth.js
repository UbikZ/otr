    ...
    http.ontimeRequestToken(req, res, function (userData) {
      User.findOne({'info.email': userData.email}).lean().exec(function (err, user) {
        if (err) {
          ...
          user.identity.ontimeToken = userData.accessToken;
          User.update({_id: user._id}, user, {}).lean().exec(function (err) {
            if (err) {
          ...
          User.update({_id: userModel._id}, userModel, {upsert: true}).lean().exec(function (err) {
            if (err) {
